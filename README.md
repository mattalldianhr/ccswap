...
Each completed cycle is recorded to `cache/usage_cycles.jsonl` (peak reached, budget left unspent, and the trajectory on that cycle's own clock, per account). Nothing reads it yet — it exists so the reserve and forecast can eventually be shaped by measurement. The accounts' windows reset at different times, so cycles are keyed per account; folding them onto one shared axis produces artifacts rather than insight.
